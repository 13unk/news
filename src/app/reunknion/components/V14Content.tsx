...
export const V14Content = () => {
  const facebookUrl = "https://www.facebook.com/profile.php?id=61586653801416";
  const bars = Array.from({ length: 24 });

  const stats = [
    { label: "Followers", value: "1.7K" },
    { label: "Views", value: "590K" },
    { label: "Likes", value: "19K" },
    { label: "Comms", value: "1.4K" },
    { label: "Shares", value: "815" },
  ];

  return (
    <div className="flex flex-col w-full h-full gap-10">
      {/* SECCIÓN ANALYTICS VISUALIZER (FACEBOOK BLUE) */}
      <motion.section 
        initial="hidden" 
        whileInView="visible" 
        ...
        <div className="flex flex-col gap-4 w-full text-white">
          ...
          <div className="w-full aspect-video bg-[#0a0a0c] rounded-2xl border border-white/10 overflow-hidden relative group shadow-2xl flex items-end justify-center px-12 pb-12">
            ...
            <div className="absolute top-0 left-0 right-0 h-8 bg-zinc-900/90 backdrop-blur border-b border-white/5 flex items-center px-4 justify-between z-20">
               ...
               <p className="text-[0.5rem] text-zinc-500 font-mono tracking-widest uppercase">metrics.facebook.analytics</p>
               <a href={facebookUrl} target="_blank" rel="noreferrer" className="hover:text-[#1877F2] transition-colors text-zinc-500">
            ...
            <div className="flex items-end gap-1 md:gap-2 w-full h-32 md:h-48">
              {bars.map((_, i) => (
                <motion.div
                  ...
                  className="flex-1 bg-gradient-to-t from-[#1877F2] to-[#3b82f6] opacity-40 group-hover:opacity-80 transition-opacity rounded-t-sm"
                />
              ))}
            </div>
